...
from .models import canonical_code, CATALOG_FIELDS

NS={'m':'http://schemas.openxmlformats.org/spreadsheetml/2006/main',
    'r':'http://schemas.openxmlformats.org/officeDocument/2006/relationships'}
ALIASES={
 'code':['코드번호','알림코드','메시지코드','메세지코드','코드','code','messagecode','id'],
 'message':['등록문구','표시문구','알림문구','메시지','메세지','문구','내용','컨텐츠','콘텐츠','message','text','contents','content'],
 'business':['업무구분','업무','business'],
 'message_type':['타입','메시지타입','메세지타입','messagetype','type'],
 'purpose':['용도','purpose'],
 'title':['타이틀','제목','title'],
 'title_en':['영문타이틀','영문제목','영문title','englishtitle','titleen'],
 'message_en':['영문컨텐츠','영문콘텐츠','영문내용','영문contents','영문content','englishcontents','messageen'],
...
def cell_coordinates(address):
    match=re.fullmatch(r'([A-Z]+)([1-9][0-9]*)',address)
    if not match: raise ImportError('셀 주소가 올바르지 않습니다.')
    column=col_index(match.group(1)); row=int(match.group(2))
    if column>=100 or row>20000:
        raise ImportError('셀 주소가 허용 범위를 벗어났습니다.')
    return column,row
...
def read_xlsx(blob):
    ...
    with z:
        ...
        for sheet in wb.findall('m:sheets/m:sheet',NS):
            ...
            root=xml(target); rows={}; formulas=set(); count=0; row_numbers=set()
            for row in root.findall('m:sheetData/m:row',NS):
                rnum=int(row.attrib.get('r','0'))
                if rnum<1 or rnum>20000: raise ImportError('행 번호가 허용 범위를 벗어났습니다.')
                if rnum in row_numbers: raise ImportError('같은 행 번호가 두 번 이상 있습니다.')
                row_numbers.add(rnum)
                vals={}
                for cell in row.findall('m:c',NS):
                    address=cell.attrib.get('r','')
                    cidx,cell_row=cell_coordinates(address)
                    if cell_row!=rnum: raise ImportError('셀 주소와 행 번호가 일치하지 않습니다.')
                    if cidx in vals: raise ImportError('같은 셀 주소가 두 번 이상 있습니다.')
                    typ=cell.attrib.get('t',''); v=cell.find('m:v',NS)
                    val=v.text if v is not None and v.text is not None else ''
                    if typ=='s':
                        try:
                            index=int(val)
                            if index<0 or index>=len(strings): raise ValueError
                            val=strings[index]
                        except ValueError as exc: raise ImportError('공유 문자열 참조가 잘못되었습니다.') from exc
...
def parse_upload(blob, filename, mapping=None):
    ...
    for sheet in sheets:
        ...
        for area in sheet['merged']:
            ...
            if len(endpoints)!=2: raise ImportError('병합 영역이 올바르지 않습니다.')
            start,end=endpoints
            c1,r1=cell_coordinates(start); c2,r2=cell_coordinates(end)
            if c2<c1 or r2<r1: raise ImportError('병합 영역의 순서가 올바르지 않습니다.')
            if r2-r1>12000: raise ImportError('병합 영역이 너무 큽니다.')
            merged.append((area,start,c1,r1,c2,r2))
        ...
        for rn in candidates:
            raw=dict(rows.get(rn,{}))
            # Vertical header labels (e.g. 업무구분 / 타입) may span both header rows.
            # Do not spread a horizontal group label such as '메시지' into Contents.
            for area,start,c1,r1,c2,r2 in merged:
                if c1==c2 and r1<rn<=r2:
                    raw.setdefault(c1,rows.get(r1,{}).get(c1,''))
                    if not raw[c1]: raw[c1]=rows.get(r1,{}).get(c1,'')
            found={}
        ...
        context_fill={}; context_sources={}
        for area,start,c1,r1,c2,r2 in merged:
            # Fill contextual metadata only; NEVER silently fan out a merged code/message.
            for field in ('menu','trigger','notes','status',*CATALOG_FIELDS):
                c=columns.get(field)
                if c is not None and c1<=c<=c2:
                    for r in range(r1,r2+1):
                        context_fill[(r,c)]=rows.get(r1,{}).get(c1,'')
                        context_sources[(r,c)]=start
        ...
        for rn,raw in sorted(rows.items()):
            ...
            values={f:str(context_fill.get((rn,c),raw.get(c,''))) for f,c in columns.items()}
            cell_map={f:context_sources.get((rn,c),col_letter(c)+str(rn)) for f,c in columns.items()}
            if not any(v.strip() for v in values.values()): continue
            loc=f'{sheet["name"]}!{rn}행'
            if any(address in sheet['formulas'] for address in cell_map.values()):
            ...
            catalog_fields={field:values.get(field,'') for field in CATALOG_FIELDS}
            limits={'business':200,'message_type':100,'purpose':2000,'title':1000,'title_en':1000,'message_en':4000}
            if any(len(catalog_fields[field])>limit for field,limit in limits.items()):
                errors.append(f'{loc}: 업무구분/타입/용도/타이틀/영문 길이 제한을 초과했습니다.'); continue
            records.append({**catalog_fields,'code':code,'message':message,'menu':values.get('menu',''),
                ...
                'source':{'filename':filename,'sheet':sheet['name'],'row':rn,
                          ...
                          'sha256':checksum,'catalog_fields':catalog_fields}})
...
def annotate_preview(parsed, db, ns):
    with db.connect() as con:
        ...
        con.execute('COMMIT')
    texts={}
    for c in existing.values(): texts.setdefault(c['message'],{})[c['code']]=None
    counters={'new':0,'unchanged':0,'conflict':0}
    for rec in parsed['records']:
        old=existing.get(rec['code'])
        if not old: rec['action']='new'
        elif all(rec.get(k,'')==old.get(k,'') for k in ('message','menu','trigger','notes','status',*CATALOG_FIELDS)): rec['action']='unchanged'
        else: rec['action']='conflict'; rec['existing']=old
        counters[rec['action']]+=1
        peers=texts.setdefault(rec['message'],{})
        rec['same_message_count']=len(peers)-(rec['code'] in peers)
        rec['same_message_codes']=list(islice((c for c in peers if c!=rec['code']),20))
        peers[rec['code']]=None

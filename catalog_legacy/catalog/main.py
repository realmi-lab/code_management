...
from .models import SearchRequest,CompareRequest,DraftRequest,ApprovalRequest,StatusRequest,ReviewRequest,ImportCommit,Namespace,canonical_code
...
def create_app(settings=None):
    settings=settings or Settings(); db=DB(settings.data_dir/'catalog.sqlite3')
    ...
    async def guard(request,call_next):
        ...
        if path.startswith('/api/') and path!='/api/health':
            ...
            if settings.access_token and not secrets.compare_digest(token,settings.access_token):
    ...
    def meta(namespace:Namespace='live'):
        with db.connect() as con:
            drafts=con.execute("SELECT COUNT(*) FROM drafts WHERE namespace=? AND state!='registered'",(namespace,)).fetchone()[0]
        ...
        codes=db.all_codes(namespace)
        return {'namespace':namespace,'counts':{'total':len(codes),'active':sum(c['status']=='active' for c in codes),'retired':sum(c['status']=='retired' for c in codes),'drafts':drafts},
          'version':db.version(namespace),'authority':'system' if namespace=='demo' else settings.authority,
    ...
    async def find(req:SearchRequest):
        return await search.find(req.query,req.namespace,req.limit,req.include_retired,req.menu,req.trigger)
    ...
    def listing(namespace:Namespace='live',q:str=Query(default='',max_length=300),status:Literal['all','active','retired']='all',offset:int=Query(0,ge=0),limit:int=Query(50,ge=1,le=500)):
        ...
        if status!='all': rows=[c for c in rows if c['status']==status]
        if q: rows=[c for c in rows if q.lower() in (' '.join([c['code'],c['message'],c['menu'],c['trigger']])).lower()]
        return {'items':rows[offset:offset+limit],'total':len(rows),'offset':offset,'limit':limit,'catalog_version':db.version(namespace)}
    ...
    def copy_code(code:str,namespace:Namespace='live'):
        ...
        return {'text':f'알림 코드: {row["code"]}\n표시 문구: {row["message"]}\n등록 메뉴: {row["menu"] or "미확인"}\n등록 노출 조건: {row["trigger"] or "미확인"}\n이번 기획 노출 조건: [기획자가 작성]\n처리 구분: 기존 코드 재사용 후보'+ ('\n주의: 실제 회사 코드가 아닌 데모입니다.' if namespace=='demo' else '')}
    ...
    async def preview(file:UploadFile=File(...),namespace:Namespace=Form('live'),mapping:str=Form('{}')):
        ...
        try:
            ...
            parsed=parse_upload(blob,file.filename or 'upload.xlsx',custom)
        ...
        with db.connect(write=True) as con:
            con.execute('INSERT INTO imports(id,namespace,filename,checksum,payload,catalog_version,created_at) VALUES(?,?,?,?,?,?,?)',
              (ident,namespace,safe_filename(file.filename or ''),hashlib.sha256(blob).hexdigest(),dumps(parsed),parsed['catalog_version'],now()))
        # Retain immutable original bytes locally; no original file is edited.
        uploads=settings.data_dir/'uploads'; uploads.mkdir(exist_ok=True)
        path=uploads/(ident+Path(parsed['filename']).suffix.lower()); path.write_bytes(blob); path.chmod(0o600)
        return parsed
    ...
    def commit_import(ident:str,req:ImportCommit):
        with db.connect(write=True) as con:
            ...
            if len(db.all_codes(ns,True,con))+data['counts']['new']>settings.max_rows:
            ...
            return {'added':count,'updated':updated,'unchanged':data['counts']['unchanged'],'skipped_conflicts':len(conflicts-approved),'catalog_version':db.version(ns,con)}
    ...
    def export_codes(namespace:Namespace='live'):
        rows=db.all_codes(namespace)
        data=csv_text(['코드번호','등록문구','사용메뉴','노출조건','비고','상태'],
          [[r['code'],r['message'],r['menu'],r['trigger'],r['notes'],'사용 중' if r['status']=='active' else '폐기'] for r in rows])
    ...
    def export_drafts(namespace:Namespace='live'):
        ...
        data=csv_text(['초안ID','상태','종류','대상코드','제안문구','메뉴','노출조건','비고','등록코드'],
           [[r['id'],r['state'],r['kind'],r['target_code'],r['message'],r['menu'],r['trigger'],r['notes'],r['registered_code']] for r in items])

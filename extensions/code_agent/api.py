...
from .inspection import snapshot as catalog_snapshot,inspect_message
...
def create_router(store,gateway,identity,enqueue=lambda:None,quota=None,sample_gateway=None,enqueue_sample=None):
    ...
    async def search_test(body:CatalogSearchTest,a=Depends(actor)):
        ...
        async with capacity:
            records,trace=await (sample_gateway if body.namespace=='demo' else gateway).search(body.query)
    ...
    def list_codes(include_retired:bool=False,offset:int=0,limit:int=100,q:str='',status:Literal['all','active','retired']|None=None,namespace:Literal['production','demo']='production',a=Depends(actor)):
        ...
        try:
            s,rows=catalog_snapshot(store,namespace,include_retired if status is None else status!='active')
        ...
        if terms:
            def matches(r):
                ...
                return all(term in text.casefold() for term in terms)
    ...
    async def compare_catalog(body:CatalogComparison,a=Depends(actor)):
        await quota.check(a.id,'review')
        async with capacity:
            return await inspect_message(store,sample_gateway if body.namespace=='demo' else gateway,body)
    @router.post('/review')
    async def review_document(body:DocumentReview,a=Depends(actor)):
        await quota.check(a.id,'review')
        version=catalog_snapshot(store,body.namespace)[0]['version']
        items=[]
        async with capacity:
            for number,line in enumerate(body.text.splitlines(),1):
                if not line.strip():continue
                result=await inspect_message(store,sample_gateway if body.namespace=='demo' else gateway,CatalogComparison(message=line,namespace=body.namespace,mode=body.mode))
    ...
    def get_code(code:str,namespace:Literal['production','demo']='production',a=Depends(actor)):
        rec=store.get_code(code) if namespace=='production' else next((r for r in catalog_snapshot(store,namespace)[1] if r['code']==code.strip().upper()),None)
    ...
    def export_json(namespace:Literal['production','demo']='production',a=Depends(actor)):
        state,items=catalog_snapshot(store,namespace)
    ...
    def export(namespace:Literal['production','demo']='production',a=Depends(actor)):
        _,items=catalog_snapshot(store,namespace); out=io.StringIO(); writer=csv.writer(out);writer.writerow(['코드번호','등록문구','메뉴','노출조건','비고','상태','업무구분','타입','용도','타이틀','영문타이틀','영문컨텐츠'])
        for rec in items:
